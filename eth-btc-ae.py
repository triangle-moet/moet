...
def trader():
    ...
    tradevarbase2 = float(float(float(float(client.get_orderbook_ticker(symbol=Crypto1)['askPrice']) * float(tradevarbase)) / float(client.get_orderbook_ticker(symbol=Crypto2)['bidPrice'])) * float(client.get_orderbook_ticker(symbol=Crypto3)['askPrice']))
